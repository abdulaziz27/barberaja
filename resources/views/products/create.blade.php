...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold mt-1">Tambah Layanan</h1>
    </div>

    <form method="POST" action="{{ route('products.store') }}" class="max-w-md space-y-4">
        @csrf
        ...
        <div>
            ...
            <input id="price" name="price" type="number" min="0" step="1000" value="{{ old('price') }}" required
        ...
        <div>
            ...
            <input id="duration_minutes" name="duration_minutes" type="number" min="1" max="480" value="{{ old('duration_minutes') }}" required
        ...
        <div>
            ...
            <select id="commission_type" name="commission_type"
                ...
                <option value="none" {{ old('commission_type', 'none') === 'none' ? 'selected' : '' }}>Tidak ada</option>
                <option value="percentage" {{ old('commission_type') === 'percentage' ? 'selected' : '' }}>Persen</option>
                <option value="fixed" {{ old('commission_type') === 'fixed' ? 'selected' : '' }}>Tetap (Rp)</option>
            </select>
        </div>

        <div>
            <label for="commission_value" class="block text-sm text-slate-400 mb-1">Nilai komisi (opsional)</label>
            <input id="commission_value" name="commission_value" type="number" min="0" step="0.01" value="{{ old('commission_value') }}"
                class="w-full rounded-lg border border-slate-700 bg-slate-900 text-slate-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
